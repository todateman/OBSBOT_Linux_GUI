...
OBSBOT SDK v2.1.0_7 で利用可能な、パンチルト操作以外の制御機能をまとめました。
...
1. [ズーム制御](#1-ズーム制御)
2. [フォーカス制御](#2-フォーカス制御)
3. [AIトラッキングモード](#3-aiトラッキングモード)
4. [カメラ設定](#4-カメラ設定)
5. [FOV(視野角)設定](#5-fov視野角設定)
6. [プリセット位置](#6-プリセット位置)
7. [HDR/WDR設定](#7-hdrwdr設定)
8. [メディアモード](#8-メディアモード)
9. [ジンバル制御モード](#9-ジンバル制御モード)
10. [デバイスステータスクエリ](#10-デバイスステータスクエリ)
11. [ジェスチャー制御](#11-ジェスチャー制御)
12. [音声制御](#12-音声制御)
13. [その他の設定機能](#13-その他の設定機能)
...
- **目的**: ゾーンプリセット位置設定前にジンバルの自動トラッキングを無効化
- **対応製品**: Tiny2シリーズ
